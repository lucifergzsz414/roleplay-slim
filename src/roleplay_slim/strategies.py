    ...
    for turn in old_turns:
        ...
        for m in turn.messages:
            ...
            if role in ("user", "assistant"):
                ...
                # Tool-call results are structured data (often JSON) —
                # trimming them with sentence-boundary regex would corrupt
                # the result. Pass through unmodified. This also keeps the
                # tool-call chain (assistant.tool_calls → tool → assistant)
                # intact rather than leaving orphaned tool_calls behind.
                new_messages.append(m)
            # Older system / developer messages (footers, memory blocks) in
            # dropped turns carry little standalone value once the turn
            # itself has been trimmed — drop them here; dedupe_verbatim_tail
            # already ran before this in the pipeline for the ones worth
            # keeping.
        if new_messages:
