...
```
  Your App               roleplay-slim                 LLM Provider
  ────────               ─────────────                 ────────────
     │                        │                             │
     │  POST /v1/chat/        │                             │
     │  completions           │                             │
     ...
     │                        │                             │
     │                  ┌─────┴─────┐                       │
     │                  │ Persona   │  never touched         │
     │                  │ prefix    │  (cache-safe)          │
     │                  └───────────┘                       │
     │                  ┌───────────┐                       │
     │                  │ Dialogue  │  whitespace            │
     │                  │ history   │  dedupe footer         │
     │                  │           │  trim old turns         │
     │                  │           │  strip directions      │
     │                  └───────────┘                       │
     │                        │                             │
...
Synthetic roleplay conversations generated by `benchmark/run_benchmark.py` —
2-message cache-stable prefix, Chinese dialogue with stage directions.
Run it yourself: `python benchmark/run_benchmark.py`.

| Turns | Before | Default (keep=6, trim) | Example config (trim + strip) |
|---|---|---|---|
| 50 | 3966 | 3366 (15.1%) | 2355 **(40.6%)** |
| 200 | 15036 | 12591 (16.3%) | 8085 **(46.2%)** |
| 500 | 37176 | 31041 (16.5%) | 19545 **(47.4%)** |

Two configs shown — the safe default (zero tuning, ~16% savings) and
`example_config.toml` with stage-direction stripping (~47% savings at
scale). The savings percentage *improves* with longer conversations
because more turns fall outside the `keep_recent_turns` window.

For apps that already run their own memory/fact-extraction layer,
`history_window_mode="drop"` + `keep_recent_turns=3` can push past
90% (see the full benchmark output for the breakdown).
