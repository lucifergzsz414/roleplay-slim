...
def test_developer_in_prefix_is_preserved():
    """developer-role messages at the start (the OpenAI-recommended
    replacement for system as of 2025) must be treated as cache-stable
    prefix, never modified by compression."""
    messages = [
    ...
    result = compress(messages, config)
    # developer message must survive untouched as prefix
    assert result[0] == {"role": "developer", "content": "you are a helpful assistant"}
    # dynamic region should have been compressed (only most recent turn survives)
    assert len(result) < len(messages)


def test_tool_call_chain_survives_compression():
    """A turn containing assistant.tool_calls → tool → assistant must stay
    intact — dropping the tool result would leave orphaned tool_calls that
    many providers reject with a 400."""
    messages = [
        {"role": "system", "content": "persona"},
        {"role": "user", "content": "what is 3+5"},
        {"role": "assistant", "content": None, "tool_calls": [
            {"id": "c1", "type": "function", "function": {"name": "calc", "arguments": '{"expr":"3+5"}'}}
        ]},
        {"role": "tool", "tool_call_id": "c1", "content": "8"},
        {"role": "assistant", "content": "3+5 equals 8"},
    ...
    config = CompressorConfig(keep_recent_turns=1, history_window_mode="trim")
    result = compress(messages, config)
    # system prefix: 1 message
    assert result[0] == {"role": "system", "content": "persona"}
    # the old tool-call turn (turn 1) should have tool and assistant messages intact
    roles = [m["role"] for m in result]
    if "tool" in roles:
        tool_idx = roles.index("tool")
        # tool message must have its content preserved (not trimmed to a stub)
        assert result[tool_idx]["content"] == "8"
        # the final assistant reply in that turn must also survive
